...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Verify this visualisation is actually a session item
  const { data: sessionItem } = await admin
    .from('session_items')
    .select('id')
    .eq('session_id', sessionId)
  ...
  if (!sessionItem) return res.status(404).json({ error: 'Visualisation not found in session' })
