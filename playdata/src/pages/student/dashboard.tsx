...
export const getServerSideProps: GetServerSideProps<Props> = async (context) => {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('id, full_name, email, username, role, education_level, created_at, onboarding_completed')
    .eq('id', user.id)
    .single();

  if (!profile) return { redirect: { destination: '/auth/login', permanent: false } };
  if (!profile.onboarding_completed) return { redirect: { destination: '/onboarding/student', permanent: false } };
